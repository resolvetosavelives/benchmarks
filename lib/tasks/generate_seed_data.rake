...
task nu_gen_activities: %i[environment] do
  ...
  benchmark_activities = activity_worksheet.drop(1).map { |row|
    cells = row.cells
    ...
    activity_types = [type_code_1, type_code_2, type_code_3].map { |type_num|
...
def crosswalk_unless_duplicate(crosswalk_to_append, crosswalks)
  dup_exists = crosswalks.any? { |crosswalk|
