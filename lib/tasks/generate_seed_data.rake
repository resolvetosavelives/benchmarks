...
desc "Generate all the JSON data: gen_evaluations gen_benchmarks gen_crosswalk gen_actions"
task gen_all: %i[
       gen_evaluations
       gen_benchmarks
       gen_crosswalk
...
desc "Generate JSON data for Influenza Actions from spreadsheet"
task gen_influenza: %i[environment] do
  influenza_actions_csv_file =
      File.join Rails.root, "/data/Influenza_Benchmarks_Crosswalk_05272020.csv"
  influenza_actions_data = []
  CSV.read(influenza_actions_csv_file).drop(1).each_with_index do |cols, idx|
    action_text = cols[8]
    benchmark_indicator_abbrev = cols[5]
    activity_type = cols[6]
    # there are some blank rows so skip those
    if action_text.present? && benchmark_indicator_abbrev.present? && activity_type.present?
      influenza_actions_data << {
  ...
  influenza_actions_file =
      File.join(Rails.root, "/db/seed-data/influenza_actions.json")
  File.open(influenza_actions_file, "w") do |f|
    f.write(JSON.pretty_generate(influenza_actions_data))
  end
  warn "Wrote %s influenza action data to file: %s" %
...
task gen_benchmarks: %i[environment] do
  fixture_spreadsheet        = File.join Rails.root, "/data/RTSL Fixtures.xlsx"
  benchmark_worksheet        =
...
task gen_crosswalk: %i[environment] do
  fixture_spreadsheet = File.join Rails.root, "/data/RTSL Fixtures.xlsx"
...
task gen_evaluations: %i[environment] do
