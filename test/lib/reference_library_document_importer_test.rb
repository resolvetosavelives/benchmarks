...
describe ReferenceLibraryDocumentImporter do
  let(:importer) { ReferenceLibraryDocumentImporter.new(csv_path) }
  let(:csv_path) { "test/fixtures/files/reference_library_documents_test.csv" }
  ...
  describe ".extract_download_url" do
    ...
    it "returns the expected URL" do
      _(
        importer.extract_download_url(attachment_field)
  ...
  describe "#record_hash_from_row" do
    ...
    let(:result) { importer.record_hash_from_row(row) }
    ...
    describe "for one indicator text" do
      before do
        BenchmarkIndicatorAction
          .expects(:find_by_text)
          .with("Give cats the vote.")
          .returns(BenchmarkIndicatorAction.first)
      end

      let(:query) { "\"1.1 Give cats the vote.\"" }
      let(:result) { importer.find_indicator_actions(query) }
