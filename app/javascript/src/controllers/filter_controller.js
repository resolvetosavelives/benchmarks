...
export default class extends Controller {
  ...
  connect() {
    if (
      $(this.multiselectTechnicalAreasTarget).siblings().find(".multiselect")
        .length === 0
    ) {
    ...
    this.allTechnicalAreaOptions = JSON.parse(
      this.multiselectTechnicalAreasTarget.getAttribute("data-options")
    )
  }
  ...
  filterByTechnicalAreas() {
    const selectedTechnicalAreaIds = $(this.multiselectTechnicalAreasTarget)
      .val()
      .map((val) => Number(val))
