...
export default class extends Controller {
    ...
    connect() {
        ...
        $(this.countrySelectTarget).chosen({no_results_text: "No countries match"})
        $(this.countrySelectTarget).on('change', this.presubmitFormToLoadOtherFields.bind(this));
        $(this.element).on("ajax:success", this.updateFormControlsFromServerResponse.bind(this))
    ...
    handleAjaxError(jqEvent, xhr, status, errMsg) {
        console.log("An error occurred while submitting the form via Ajax: ", errMsg)
    }
    ...
    selectAllTechnicalAreasForJee1(event) {
        event.preventDefault()
        event.stopPropagation()
        $(this.cardOfTechnicalAreasForJee1Target).find("input[type=checkbox]").prop("checked", true)
    }

    deselectAllTechnicalAreasForJee1(event) {
        event.preventDefault()
        event.stopPropagation()
        $(this.cardOfTechnicalAreasForJee1Target).find("input[type=checkbox]").prop("checked", false)
    }

    selectAllTechnicalAreasForSpar2018(event) {
        event.preventDefault()
        event.stopPropagation()
        $(this.cardOfTechnicalAreasForSpar2018Target).find("input[type=checkbox]").prop("checked", true)
    }

    deselectAllTechnicalAreasForSpar2018(event) {
        event.preventDefault()
        event.stopPropagation()
        $(this.cardOfTechnicalAreasForSpar2018Target).find("input[type=checkbox]").prop("checked", false)
    }
}
