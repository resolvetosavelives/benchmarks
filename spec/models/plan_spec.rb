...
describe(Plan) do
  ...
  describe("#is_5_year?") do
    ...
    it("is false by default") { _(subject.is_5_year?).must_equal(false) }
  ...
  describe(".new_from_assessment") do
    describe("for Nigeria JEE 5-year plan") do
      ...
      it("has the expected term") { _(subject.term).must_equal(500) }
    end
  end
  describe(".create_from_goal_form") do
    describe("for Nigeria JEE1") do
      ...
      it("has the expected name") { _(plan.name).must_equal("test plan 3854") }
    ...
    describe("for Nigeria SPAR 2018") do
      ...
      it("has the expected name") { expect(plan.name).to(be_truthy) }
      it("has the expected number of indicators") do
        expect(plan.goals.size).to(eq(44))
      end
      it("has the expected number of actions") do
        expect(plan.plan_actions.size).to(eq(354))
      end
    end
    describe("for Nigeria from technical areas 5-year") do
      ...
      it("has the expected number of actions") do
        expect(plan.plan_actions.size).to(eq(205))
      end
    end
    describe("for Nigeria JEE1 with influenza") do
      ...
      it("has the expected name") { expect(plan.name).to(eq("test plan 3854")) }
      it("has the expected number of indicators") do
        expect(plan.goals.size).to(eq(44))
      end
      it("has the expected number of actions") do
        expect(plan.plan_actions.size).to(eq(407))
      end
    ...
    describe("for Nigeria JEE1 with influenza and cholera") do
      ...
      it(
        "influenza actions and cholera actions have been assigned to the appropriate indicator"
      ) do
        indicator = BenchmarkIndicator.first
        actions_for_indicator_count =
          BenchmarkIndicatorAction.where(
    ...
    describe("for Nigeria JEE1 with influenza, cholera, and ebola") do
      let(:plan) do
        Plan.create_from_goal_form(
          ...
          plan_name: "test plan 3854",
          disease_ids:
            ([Disease.influenza.id, Disease.cholera.id, Disease.ebola.id])
        )
      end
      it("returns a saved plan instance") do
        expect(plan.persisted?).to(eq(true))
      ...
      it(
        "influenza, cholera, and ebola actions have been assigned to the appropriate indicator"
      ) do
        indicator = BenchmarkIndicator.first
        actions_for_indicator_count =
          BenchmarkIndicatorAction.where(
      ...
      it("has associated diseases influenza, cholera, and ebola") do
        expect([Disease.influenza, Disease.cholera, Disease.ebola]).to(
          eq(plan.diseases)
        )
      end
    end
