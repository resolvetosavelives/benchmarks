...
(echo "Migrating Database..." && bundle exec bin/rails db:schema:loaded db:migrate 2>/dev/null) || (echo "Creating database..." && bundle exec bin/rails db:create db:environment:set db:schema:load db:seed)
